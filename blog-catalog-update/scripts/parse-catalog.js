 ...
 *
 * 用法：
 *   node parse-catalog.js --posts-dir <source/_posts路径> --output <catalog.json输出路径> [--catalog-md <总目录.md路径>]
 *
 * 示例：
 *   node parse-catalog.js \
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      case '--posts-dir':
        result.postsDir = args[++i];
        break;
      case '--output':
        result.output = args[++i];
        break;
      case '--catalog-md':
        result.catalogMd = args[++i];
        break;
      case '-h':
      case '--help':
        console.log('用法: node parse-catalog.js --posts-dir <source/_posts路径> --output <catalog.json输出路径> [--catalog-md <总目录.md路径>]');
        console.log('');
        console.log('参数:');
        console.log('  --posts-dir    source/_posts 目录路径（必需）');
        console.log('  --output       catalog.json 输出路径（必需）');
        console.log('  --catalog-md   总目录.md 路径（可选，默认为 posts-dir/总目录.md）');
        process.exit(0);
      default:
  ...
  if (!result.postsDir) {
    console.error('错误: 缺少 --posts-dir 参数');
    process.exit(1);
  }
  if (!result.output) {
  ...
  // 默认 catalog-md 为 posts-dir/总目录.md
  if (!result.catalogMd) {
    result.catalogMd = path.join(result.postsDir, '总目录.md');
  }
...
const POSTS_DIR = path.resolve(config.postsDir);
const OUTPUT_FILE = path.resolve(config.output);
const CATALOG_MD = path.resolve(config.catalogMd);
