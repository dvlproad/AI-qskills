#!/usr/bin/env node
/**
 * 总目录生成器（参数化版本）— 将 catalog.json 渲染为 总目录.md
 *
 * 用法:
 *   node json-to-md.js --input <catalog.json> --output <总目录.md> [--stdout] [--remove-secNum-at-first true]
 *
 * 示例:
 *   node json-to-md.js \
 *     --input /Users/qian/Project/dvlproadHexo/source/_posts/总目录/data/catalog.json \
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--stdout':
        result.stdout = true;
        break;
      case '--remove-secNum-at-first':
        result.removeSecNumAtFirst = args[++i] === 'true';
        break;
      case '-h':
      case '--help':
        console.log('用法: node json-to-md.js --input <catalog.json> --output <总目录.md> [--stdout] [--remove-secNum-at-first true]');
        console.log('');
        console.log('参数:');
        console.log('  --input    catalog.json 输入路径（必需）');
        console.log('  --output   总目录.md 输出路径（必需）');
        console.log('  --stdout   输出到控制台而不是文件');
        console.log('  --remove-secNum-at-first true   清空所有 secNum 后全走自动编号');
        process.exit(0);
      default:
  ...
  if (!result.output && !result.stdout) {
    console.error('错误: 缺少 --output 参数（或使用 --stdout）');
    process.exit(1);
  }
...
const CATALOG_MD = config.output ? path.resolve(config.output) : null;
...
function readFrontmatter() {
  if (!CATALOG_MD || !fs.existsSync(CATALOG_MD)) return null;
