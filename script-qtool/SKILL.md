...
- `qtool提交代码` - 规范化Git提交
- `qtool pgyer` / `qtool 蒲公英` - 上传到蒲公英等平台
- `qtool gui` / `qtool生成app` / `qtool构建gui` - 构建 Qtool.app
- `qtool帮助` - 显示帮助
- 其他包含 "qtool" 的指令
...
| 指令 | 说明 | 命令 |
|------|------|------|
| qtool gui / qtool生成app / qtool构建gui | 构建 Qtool.app | `sh {SCRIPT_DIR}/gui/build.sh` |
...
> 构建时会询问输出路径：`yes`=桌面 / 输入路径 / 回车跳过。

```bash
SCRIPT_DIR=/Users/lichaoqian/Project/CQCI/script-branch-json-file
sh "$SCRIPT_DIR/gui/build.sh"
```

构建产物：`gui/Qtool`（二进制）和 `gui/Qtool.app`（macOS 应用包）。

```bash
# 运行
open "$SCRIPT_DIR/gui/Qtool.app"
```
