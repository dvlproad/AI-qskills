...
```bash
sh organize-pod-to-md/scripts/github_pod_all.sh
```
...
| 功能 | 说明 |
|------|------|
| 脚本路径 | `organize-pod-to-md/scripts/github_pod_all.sh` |
...
1. **需要 CocoaPods 环境** — `github_pod_all.sh` 依赖 `pod` 命令
...
5. **匹配数据源** — 必须先运行 `github_pod_all.sh` 生成 JSON，再运行 `pod_match2_repos.sh`

## 版本记录

### 0.0.2 (2026-05-10): 新增私有 Pod 扫描、来源/可见/语言列
- `github_pod_all.sh`: 新增扫描 `dvlproadSpecs` 私有 repo 的 `.podspec` 文件
...
- `github_pod_all.sh`: 获取所有 pod 数据，输出 md + json
