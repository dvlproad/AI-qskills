...
```bash
sh organize-pod-to-md/scripts/pods_collectAndRender.sh
```

生成文件（在当前工作目录）：
- `github_pod_all.md` — 给人看的表格
- `github_pod_all.json` — 给脚本用的结构化数据
...
### pods_collectAndRender.sh

| 功能 | 说明 |
|------|------|
| 脚本路径 | `organize-pod-to-md/scripts/pods_collectAndRender.sh` |
| 公有数据源 | `pod trunk me` + 本地 trunk/cocoapods repo 缓存 |
| 私有数据源 | 扫描 `gitee-dvlproad-dvlproadspecs` 的全部 `.podspec`（Ruby 格式，Python 正则解析） |
| 输出 md | `github_pod_all.md` — 7 列表格（Pod、Version、Git URL、Source、Visibility、Language） |
| 输出 json | `github_pod_all.json` — 结构化数据（pod、version、git、summary、source、visibility、language） |
...
1. **需要 CocoaPods 环境** — `pods_collectAndRender.sh` 依赖 `pod` 命令
...
5. **匹配数据源** — 必须先运行 `pods_collectAndRender.sh` 生成 JSON，再运行 `pod_match2_repos.sh`

## 版本记录

### 0.0.2 (2026-05-10): 新增私有 Pod 扫描、来源/可见/语言列
- `pods_collectAndRender.sh`: 新增扫描 `dvlproadSpecs` 私有 repo 的 `.podspec` 文件
...
- `pods_collectAndRender.sh`: 获取所有 pod 数据，输出 md + json
