...
各阶段适用环境：
- **Phase 1 (fetch)**：仅 HTTP 走（file:// 因 CORS 跨域一定失败）
- **Phase 2 (JS)**：仅本地 file:// 走（强制用户不要将 .js 文件部署到远程）
- **Phase 3 (文件选择器)**：作为降级兜底，不限环境。用户主动选择文件即表示数据在本地，可配合双按钮支持"显示所有/仅可见"区分
...
Phase 1: fetch('data.json')
  ├── file:// 本地双击 → CORS 跨域，一定失败 → 进入 Phase 2
  └── HTTP 环境
        ├── 成功 → startApp(data) → 替换为完整数据 ✅
        └── 失败（文件不存在等）→ 进入 Phase 2

Phase 2: 动态 <script> 加载 data.js（⚠️ .js 文件永远不要部署到远程服务器，有一份json就够了）
  ├── HTTP 远程 → .js 文件不存在，一定失败 → 进入 Phase 3
  └── file:// 本地
        ├── 未创建 .js 文件 → 一定失败 → 进入 Phase 3
        └── 已通过命令创建 .js 文件
              ├── 成功（window.DATA 有值）→ startApp(DATA) → 完整渲染 ✅
              └── 失败（.js 文件损坏等）→ 进入 Phase 3

注：.js 文件本质是 .json 的冗余副本。（第一行写 `window.DATA = `，后面拼接完整 JSON 内容。这样 `<script>` 标签加载后，`window.DATA` 就是可直接使用的 JS 对象。）

注：
- **本地 file://**：如果有"显示所有/仅可见"需求，可在 Phase 2 加载成功后读取 URL 参数 `?showAll=true` 控制 startApp 的过滤行为，显示全部数据（详见代码模板）。远程不走 Phase 2。
- **远程 HTTP**：不建议有"显示所有/仅可见"需求，为的是始终过滤隐藏数据以保护隐私。（如确需支持，可在 Phase 1 中同样通过 URL 参数控制。）
