...
Phase 2: 动态 <script> 加载 data.js（file:// 和 HTTP 均可工作，不受 CORS 限制）
  ├── .js 文件存在且 window.DATA 有值 → startApp(DATA) → 完整渲染 ✅
  └── .js 文件不存在 → 加载失败 → 进入 Phase 3

注：如有"显示所有/仅可见"需求，可在 Phase 2 加载成功后读取 URL 参数 `?showAll=true` 控制 startApp 的过滤行为（详见代码模板）。注意安全边界：
- **本地 file://**：数据在本机，无隐私顾虑，可通过 `?showAll=true` 显示全部数据
- **远程 HTTP**：数据公开访问，不建议读取该参数，始终过滤隐藏数据以保护隐私
