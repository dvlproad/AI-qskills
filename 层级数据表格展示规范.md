...
- **优先级链**：此处实现的是抽象《优先级链模式》的具体实例，模型为 `自身 > 父级 > 全局` 三层。显示时高优先覆盖低优先；修改低优先时自动清除高优先。详见《[优先级链模式.md](./优先级链模式.md)》
- 该 Segmented Control UI 模式是通用优先级链模式的具象化表达，各档位对应优先级层索引，填充规则遵循累进亮起原则
- 局部 DOM 替换避免全量重渲染
...
此处是通用《[优先级链模式.md](./优先级链模式.md)》在 dvlproad 项目列表中的具体实现。三层叠配置：**全局 toolbar（层 0）→父分类 seg（层 1）→自身 seg（层 2）**。
...
> **联动 N层规则**：以上 priority chain 跟踪的是档位级别的读写清除。在此之上，手动展开/收起状态遵循 N层嵌套规则的加减档规则：减档时被减掉层级的展开状态全部丢弃，加档回本层时恢复默认。完整规范见《[优先级链模式.md](./优先级链模式.md#N层嵌套表格层级展开规则说明)》的「N层嵌套表格层级展开规则说明」一节。
...
**注意**：手动展开状态不跨档位保留，加/减档时重置。完整规范见《[优先级链模式.md](./优先级链模式.md)》中的 N层嵌套表格层级展开规则说明。
...
**`dvlproad项目列表.html`** 是本方案的完整实现，位于：
```
