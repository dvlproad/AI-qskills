...
- `organize-pod-to-md/scripts/` — 最复杂的脚本，分两步：
  ...
  2. `pod_match2_repos.sh` — 把 pod 数据匹配到项目列表的 markdown 表格中
...
- `pod_match2_repos.sh` 是用 heredoc 内嵌 Python3 的脚本，不依赖外部 Python 包，只用 `json`/`re`/`sys`。
...
sh organize-pod-to-md/scripts/pod_match2_repos.sh --subspec-min-count <N> [--subspec-force-show PodA,PodB] [--separate-subspecs] <项目列表.md> <pod_json.json>
```
