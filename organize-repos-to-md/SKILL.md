...
repos_all.json 生成后，可按第 8 节的 Markdown 格式渲染为项目列表文档。

遍历 repos_all.json，按 type 深度输出 heading → intro → values 表格，即可得到完整的 dvlproad项目列表.md。
...
输出 `repos_all.json`，供 `organize-pod-to-md` 消费。JSON 结构参见 3.1 节，字段说明：
